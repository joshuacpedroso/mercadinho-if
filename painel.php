...
const SALES_DASHBOARD_API_URL = 'https://n8n.italianofacil.com.br/webhook/dashboard-vendas-resumo';
...
function normalize_goal_name(string $value): string
{
...
function parse_dashboard_money($value): float
{
...
function daily_sales_goal_config(): array
{
    return [
        [
            'name' => 'Gabriel',
            'email' => 'gabrielmartins@italianofacil.com',
            'goal' => 8190.0,
            'aliases' => ['Gabriel', 'Gabriel Martins'],
        ],
        [
            'name' => 'Heloisa',
            'email' => 'heloisa@italianofacil.com',
            'goal' => 7300.0,
            'aliases' => ['Heloisa', 'Heloísa'],
        ],
        [
            'name' => 'Marina',
            'email' => 'marina@italianofacil.com',
            'goal' => 7300.0,
            'aliases' => ['Marina'],
        ],
        [
            'name' => 'Alana',
            'email' => 'alana@italianofacil.com',
            'goal' => 6350.0,
            'aliases' => ['Alana'],
        ],
        [
            'name' => 'Victoria',
            'email' => 'victoria@italianofacil.com',
            'goal' => 6350.0,
            'aliases' => ['Victoria', 'Vitória', 'Vitoria'],
        ],
        [
            'name' => 'Bruna',
            'email' => 'bruna@italianofacil.com',
            'goal' => 6350.0,
            'aliases' => ['Bruna'],
        ],
    ];
...
function daily_sales_goal_vendor_emails(): array
{
    $emails = [];

    foreach (daily_sales_goal_config() as $config) {
        $email = normalize_email((string) ($config['email'] ?? ''));
        if ($email !== '') {
            $emails[] = $email;
        }
    }
...
function is_daily_sales_goal_vendor_user(array $user): bool
{
    ...
    return in_array($email, daily_sales_goal_vendor_emails(), true);
}

function find_active_sales_goal_vendor_by_email(string $email): ?array
{
    $stmt = db()->prepare('SELECT * FROM users WHERE email = ? AND active = 1 LIMIT 1');
    $stmt->execute([normalize_email($email)]);
    $user = $stmt->fetch();

    if (!$user || !is_daily_sales_goal_vendor_user($user)) {
        return null;
    }
...
function fetch_sales_dashboard_data(): ?array
{
    $json = null;

    if (function_exists('curl_init')) {
        $curl = curl_init(SALES_DASHBOARD_API_URL);
        curl_setopt_array($curl, [
            ...
            CURLOPT_USERAGENT => 'MercadinhoIF/1.0',
        ]);
        ...
        if (is_string($response) && $response !== '' && $status >= 200 && $status < 300) {
            ...
            error_log('[Mercadinho IF][Meta Automatica] Erro cURL: HTTP ' . $status . ' ' . $error);
        }
    }
    ...
    if ($json === null) {
        $context = stream_context_create([
            'http' => [
                'timeout' => 12,
                'ignore_errors' => true,
                'header' => "User-Agent: MercadinhoIF/1.0\r\n",
            ],
        ]);

        $response = @file_get_contents(SALES_DASHBOARD_API_URL, false, $context);
    ...
    if ($json === null) {
        setting_set('last_daily_sales_goal_check', json_encode([
            'ok' => false,
            'error' => 'Não foi possível buscar o dashboard comercial.',
            'checked_at' => now(),
    ...
    if (!is_array($data) || !empty($data['isError'])) {
        setting_set('last_daily_sales_goal_check', json_encode([
            'ok' => false,
            'error' => 'Dashboard retornou JSON inválido ou com erro.',
            'checked_at' => now(),
        ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
        return null;
    }
...
function sales_goal_date_key(array $data): string
{
    $date = trim((string) ($data['debugHoje'] ?? $data['comDataFev'] ?? $data['dataHoje'] ?? $data['hoje'] ?? ''));
...
function dashboard_day_rows_from_value($value): array
{
    if (!is_array($value)) {
        return [];
    }
    ...
    foreach ($value as $name => $amount) {
        if (is_array($amount)) {
            $row = $amount;
            if (!isset($row['nome']) && !isset($row['name']) && !isset($row['vendedor'])) {
                $row['nome'] = (string) $name;
            }
            $rows[] = $row;
        } else {
            $rows[] = [
...
function dashboard_sales_day_rows(array $data): array
{
    $candidateKeys = [
        'topVendasDia',
        'vendasDia',
        ...
        'todaySales',
        'fatDiaPorVendedor',
        'faturamentoDiaPorVendedor',
        'faturamentoHojePorVendedor',
    ];
    ...
    foreach ($candidateKeys as $key) {
        ...
        $rows = dashboard_day_rows_from_value($data[$key]);
        if ($rows) {
            return $rows;
        }
    }
...
function dashboard_row_name(array $row): string
{
    foreach (['nome', 'name', 'vendedor', 'seller', 'responsavel', 'colaborador'] as $key) {
        if (!empty($row[$key])) {
...
function dashboard_row_day_value(array $row): float
{
    foreach ([
        'valor',
        'valorDia',
        ...
        if (array_key_exists($key, $row)) {
            return parse_dashboard_money($row[$key]);
        }
    }

    foreach (['valorFormatado', 'valorDiaFormatado', 'fatDiaFormatado', 'faturamentoDiaFormatado', 'vendasHojeFormatado'] as $key) {
        if (array_key_exists($key, $row)) {
            return parse_dashboard_money($row[$key]);
        }
    }
...
function dashboard_sales_by_person(array $data): array
{
    $sales = [];
    $rows = dashboard_sales_day_rows($data);
    ...
    foreach ($rows as $row) {
        ...
        $key = normalize_goal_name(dashboard_row_name($row));
        if ($key === '') {
            continue;
        }

        $value = dashboard_row_day_value($row);
        $sales[$key] = ($sales[$key] ?? 0.0) + $value;
    }

    return $sales;
}

function daily_sales_for_config(array $sales, array $config): float
{
    $total = 0.0;
    $used = [];

    foreach (($config['aliases'] ?? []) as $alias) {
        $key = normalize_goal_name((string) $alias);
...
function mark_daily_sales_reward_if_available(string $settingKey, array $payload): bool
{
    $json = json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);

    $stmt = db()->prepare('INSERT OR IGNORE INTO app_settings (key, value, updated_at) VALUES (?, ?, ?)');
    $stmt->execute([$settingKey, $json ?: '{}', now()]);

    return $stmt->rowCount() > 0;
}

function run_daily_sales_goal_rewards_if_needed(): void
{
    try {
        $data = fetch_sales_dashboard_data();

        if (!$data) {
            return;
        }

        $dateKey = sales_goal_date_key($data);
        $sales = dashboard_sales_by_person($data);
        $debugRewards = [];

        if (!$sales) {
            setting_set('last_daily_sales_goal_check', json_encode([
                'ok' => false,
                'error' => 'Nenhuma venda do dia encontrada no retorno do dashboard comercial.',
                'date_key' => $dateKey,
                'checked_at' => now(),
            ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
            return;
        ...
        foreach (daily_sales_goal_config() as $config) {
            $email = normalize_email((string) $config['email']);
            $soldToday = daily_sales_for_config($sales, $config);
            ...
            $settingKey = 'daily_sales_goal_reward_' . $dateKey . '_' . $email;
            $recipient = find_active_sales_goal_vendor_by_email($email);

            if (!$recipient) {
                error_log('[Mercadinho IF][Meta Automatica] Vendedor ativo não encontrado ou fora da lista permitida: ' . $email);
                continue;
            }
            ...
            try {
                $locked = mark_daily_sales_reward_if_available($settingKey, [
                    'email' => $email,
                    'name' => $config['name'],
                    'sold_today' => $soldToday,
                    'goal' => $goal,
                    'source' => 'dashboard-comercial-n8n',
                    'rewarded_at' => now(),
                ]);
                ...
                $newBalance = (int) $recipient['balance'] + 1;
                $reason = 'META DIÁRIA BATIDA - RECARGA AUTOMÁTICA';

                $pdo->prepare('UPDATE users SET balance = balance + 1, updated_at = ? WHERE id = ?')
                    ->execute([now(), (int) $recipient['id']]);

                $pdo->prepare('INSERT INTO credit_logs (user_id, admin_id, amount, reason, created_at) VALUES (?, NULL, 1, ?, ?)')
                    ->execute([(int) $recipient['id'], $reason, now()]);
                ...
                error_log('[Mercadinho IF][Meta Automatica] Erro ao premiar ' . $email . ': ' . $exception->getMessage());
                continue;
            }

            try {
                send_ifcoins_email($recipient, 1, 'META DIÁRIA BATIDA - RECARGA AUTOMÁTICA', $newBalance, 'daily_goal');
            } catch (Throwable $mailError) {
                error_log('[Mercadinho IF][Meta Automatica Email] ' . $mailError->getMessage());
            }
        }
        ...
        error_log('[Mercadinho IF][Meta Automatica] Erro geral: ' . $exception->getMessage());
        try {
            setting_set('last_daily_sales_goal_check', json_encode([
                'ok' => false,
            ...
            // Evita quebrar o painel caso até o log falhe.
        }
    }
}
...
function coin_request_stickers(): array
{
    return [
        'humilde' => [
            'label' => 'Pedido humilde',
            'image' => 'https://i.imgur.com/w92H9b2.png',
        ],
        'moedinha' => [
            'label' => 'Só uma moedinha',
            'image' => 'https://i.imgur.com/RKMJjlx.png',
        ],
        'ajuda' => [
            'label' => 'Ajuda o colega',
            'image' => 'https://i.imgur.com/GoCMQ25.png',
        ],
        'pobre' => [
            'label' => 'Tô pobre de IF Coin',
            'image' => 'https://i.imgur.com/cc0MRGA.png',
        ],
        'emergencia' => [
            'label' => 'Emergência no mercadinho',
            'image' => 'https://i.imgur.com/3CTRXvy.png',
        ],
    ];
}

function coin_request_sticker(string $key): array
{
    $stickers = coin_request_stickers();
    return $stickers[$key] ?? $stickers['humilde'];
}

function send_ifcoins_request_email(array $target, array $requester, int $requestId, int $amount, string $message, string $stickerKey): void
{
    $sticker = coin_request_sticker($stickerKey);

    $safeTarget = e((string) $target['name']);
    $safeRequester = e((string) $requester['name']);
    $safeMessage = e($message !== '' ? $message : 'Me doa 1 IF Coin? kkkkk');
    $safeStickerImage = e((string) $sticker['image']);
    $safeStickerLabel = e((string) $sticker['label']);

    $donateUrl = MERCADINHO_URL . 'painel.php?page=conta'
        . '&donate_to=' . urlencode((string) $requester['code'])
        . '&amount=' . $amount
        . '&request_id=' . $requestId
        . '&message=' . urlencode('Pedido atendido!');

    $safeDonateUrl = e($donateUrl);

    $amountText = $amount . ' IF Coin' . ($amount === 1 ? '' : 's');
    $subject = $requester['name'] . ' pediu ' . $amountText . ' para você';

    $text = "Oi, {$target['name']}!\n\n"
        . "{$requester['name']} pediu {$amountText} para você.\n"
        . "Mensagem: {$message}\n\n"
        . "Para doar, acesse:\n"
        . $donateUrl . "\n\n"
        . "A doação não é automática. Você confirma antes de enviar.";
    ...
    $bodyHtml = '
        ...
        <div style="text-align:center;margin:0 0 24px;">
            <img src="' . $safeStickerImage . '" alt="' . $safeStickerLabel . '" style="width:150px;max-width:150px;height:auto;display:block;margin:0 auto 14px;border-radius:22px;">
            <h2 style="margin:0;color:#132019;font-size:26px;line-height:1.15;font-weight:900;">
                ' . $safeRequester . ' pediu uma moedinha kkkkk
            </h2>
            <p style="margin:10px 0 0;color:#526057;font-size:16px;line-height:1.5;">
                Oi, <strong>' . $safeTarget . '</strong>! Você recebeu um pedido de doação no Mercadinho IF.
            </p>
        </div>
        ...
        <div style="background:linear-gradient(180deg,#fff8df,#f4edcf);border:1px solid #e5d49a;border-radius:24px;padding:24px;text-align:center;margin:22px 0;box-shadow:0 18px 42px rgba(19,32,25,.10);">
            ...
            <div style="font-size:56px;font-weight:900;color:#0b7a42;line-height:1;margin:10px 0;text-shadow:0 8px 24px rgba(11,122,66,.18);">
                +' . $amount . '
            </div>

            <div style="font-size:15px;color:#132019;font-weight:800;">
                ' . $safeRequester . ' quer ' . $amountText . '
            </div>
        </div>

        <div style="background:#f8f5ea;border:1px solid #eadfbe;border-radius:18px;padding:16px 18px;margin:18px 0;">
            <div style="font-size:13px;color:#687369;line-height:1.5;">
        ...
        <div style="text-align:center;margin:26px 0 10px;">
            <a href="' . $safeDonateUrl . '" style="display:inline-block;background:#0f8f4f;color:#ffffff;text-decoration:none;padding:15px 24px;border-radius:14px;font-weight:900;font-size:15px;box-shadow:0 10px 24px rgba(15,143,79,.25);">
                Doar 1 IF Coin
            </a>
        </div>
    ';
    ...
    $html = email_shell(
        'IF Coins • Pedido recebido',
        'Pedido de moedinha',
        'Alguém da equipe pediu uma força no Mercadinho IF.',
        $bodyHtml,
        '#0b7a42',
        false
    );

    smtp_send((string) $target['email'], (string) $target['name'], $subject, $html, $text);
}
...
function schema(): void
{
    ...
    db()->exec("
        ...
        CREATE TABLE IF NOT EXISTS coin_requests (
            ...
            message TEXT,
            sticker_key TEXT NOT NULL DEFAULT 'humilde',
    ...
    if (!column_exists('transactions', 'original_product_id')) {
        db()->exec("ALTER TABLE transactions ADD COLUMN original_product_id INTEGER");
    }
    if (!column_exists('transactions', 'original_product_name')) {
        db()->exec("ALTER TABLE transactions ADD COLUMN original_product_name TEXT");
    }
    if (!column_exists('transactions', 'adjusted_at')) {
        db()->exec("ALTER TABLE transactions ADD COLUMN adjusted_at TEXT");
    }
    if (!column_exists('transactions', 'adjusted_note')) {
        db()->exec("ALTER TABLE transactions ADD COLUMN adjusted_note TEXT");
    }
}
...
function handle_post(): void
{
    ...
    try {
        ...
        if ($action === 'adjust_redemption') {
            $accountUser = current_user();
            if (!$accountUser) {
                throw new RuntimeException('Entre na sua conta para ajustar um resgate.');
            }

            $transactionId = (int) ($_POST['transaction_id'] ?? 0);
            $newProductId = (int) ($_POST['new_product_id'] ?? 0);

            if ($transactionId <= 0 || $newProductId <= 0) {
                throw new RuntimeException('Escolha o resgate e o produto substituto.');
            }
            ...
            $stmt->execute([$transactionId, (int) $accountUser['id']]);
            ...
            $stmt->execute([(int) $accountUser['id']]);
            $freshAccountUser = $stmt->fetch();

            if (!$transaction) {
                throw new RuntimeException('Resgate não encontrado para ajustar.');
            }
            if (!$newProduct) {
                throw new RuntimeException('Produto substituto não encontrado ou inativo.');
            }
            if (!$freshAccountUser) {
                throw new RuntimeException('Sua conta não está ativa para ajustar resgates.');
            }
            if ((int) $transaction['product_id'] === (int) $newProduct['id']) {
                throw new RuntimeException('Escolha um produto diferente do resgate original.');
            }
            if ((int) $newProduct['stock'] <= 0) {
                throw new RuntimeException('Esse produto substituto também está sem estoque.');
            }

            $oldCost = (int) $transaction['cost'];
            $newCost = (int) $newProduct['cost'];
            $difference = $newCost - $oldCost;

            if ($difference > 0 && (int) $freshAccountUser['balance'] < $difference) {
                throw new RuntimeException('Esse produto custa mais IF Coins e seu saldo não cobre a diferença.');
            }

            $oldProductId = (int) $transaction['product_id'];
            $oldProductName = (string) $transaction['product_name'];
            $note = 'Ajuste de resgate: ' . $oldProductName . ' → ' . (string) $newProduct['name'];

            // Não devolve estoque para o item original.
            // Se a pessoa ajustou o resgate, é porque chegou no frigobar e esse item não existia fisicamente.
            // Então o produto original deve continuar zerado/baixado no sistema.
            $pdo->prepare('UPDATE products SET stock = stock - 1, updated_at = ? WHERE id = ? AND stock > 0')
                ->execute([now(), (int) $newProduct['id']]);

            if ($difference !== 0) {
                $pdo->prepare('UPDATE users SET balance = balance - ?, updated_at = ? WHERE id = ?')
                    ->execute([$difference, now(), (int) $freshAccountUser['id']]);

                $reason = $difference > 0
                    ? 'Ajuste de resgate: diferença cobrada'
                    : 'Ajuste de resgate: diferença devolvida';

                $pdo->prepare('INSERT INTO credit_logs (user_id, admin_id, amount, reason, created_at) VALUES (?, NULL, ?, ?, ?)')
                    ->execute([(int) $freshAccountUser['id'], -$difference, $reason . ' (' . $oldProductName . ' → ' . (string) $newProduct['name'] . ')', now()]);
            }

            $pdo->prepare('
                UPDATE transactions
                SET original_product_id = COALESCE(original_product_id, ?),
                    original_product_name = COALESCE(original_product_name, ?),
                    product_id = ?,
                    product_name = ?,
                    cost = ?,
                    adjusted_at = ?,
            ...
            $coinNote = '';
            if ($difference > 0) {
                $coinNote = ' Foi cobrada a diferença de ' . $difference . ' IF Coin' . ($difference > 1 ? 's' : '') . '.';
            } elseif ($difference < 0) {
                $refund = abs($difference);
                $coinNote = ' Voltou ' . $refund . ' IF Coin' . ($refund > 1 ? 's' : '') . ' para sua conta.';
            }

            flash('Resgate ajustado! O item que não tinha ficou baixado, e o produto que você pegou foi descontado.' . $coinNote);
            redirect_to('painel.php?page=conta&adjusted=1');
        }
        ...
        if ($action === 'donate_coins') {
            ...
            if ($requestId > 0) {
                $stmt = $pdo->prepare('SELECT * FROM coin_requests WHERE id = ? AND status = ?');
                $stmt->execute([$requestId, 'pending']);
                $coinRequest = $stmt->fetch();
            
                if (
                    $coinRequest
                    && (int) $coinRequest['requester_id'] === (int) $recipient['id']
                    && (int) $coinRequest['target_id'] === (int) $donorFresh['id']
                ) {
                    $pdo->prepare('UPDATE coin_requests SET status = ?, fulfilled_at = ? WHERE id = ?')
                        ->execute(['fulfilled', now(), $requestId]);
                }
            }
        ...
        if ($action === 'request_coins') {
            ...
            if (!$requester) {
                throw new RuntimeException('Entre na sua conta para pedir IF Coins.');
            }
        
            $targetCode = strtoupper(trim((string) ($_POST['target_code'] ?? '')));
            $message = trim((string) ($_POST['message'] ?? 'Me doa 1 IF Coin? Nunca te pedi nada... hoje pedi kkkkk'));
            $stickerKey = trim((string) ($_POST['sticker_key'] ?? 'humilde'));
            $amount = 1;
        
            if (!preg_match('/^[A-Z0-9]{5}$/', $targetCode)) {
                throw new RuntimeException('Informe um código válido com 5 caracteres.');
            }
        
            if ((function_exists('mb_strlen') ? mb_strlen($message) : strlen($message)) > 180) {
                throw new RuntimeException('A mensagem do pedido pode ter no máximo 180 caracteres.');
            }
        
            $target = find_code_user($targetCode);
            ...
            if ((int) $target['id'] === (int) $requester['id']) {
                throw new RuntimeException('Você não pode pedir IF Coin para si mesmo kkkkk.');
            }
        
            if ((int) $target['balance'] < 1) {
                throw new RuntimeException('Essa pessoa está sem IF Coins para doar agora.');
            }
            ...
            ");
            $stmt->execute([(int) $requester['id']]);
        
            if ((int) $stmt->fetchColumn() >= 3) {
                throw new RuntimeException('Você já fez 3 pedidos hoje. Amanhã pode pedir de novo kkkkk.');
            }
            ...
            try {
                $pdo->prepare('
                    INSERT INTO coin_requests (
                        requester_id, target_id, requester_name, target_name, requester_code,
                        amount, message, sticker_key, status, email_status, created_at
                    )
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ')->execute([
                    (int) $requester['id'],
            ...
            try {
                send_ifcoins_request_email($target, $requester, $requestId, $amount, $message, $stickerKey);
                db()->prepare('UPDATE coin_requests SET email_status = ? WHERE id = ?')
                    ->execute(['sent', $requestId]);
        
                $mailNote = ' Email de pedido enviado.';
            } catch (Throwable $mailError) {
                db()->prepare('UPDATE coin_requests SET email_status = ? WHERE id = ?')
                    ->execute(['failed: ' . $mailError->getMessage(), $requestId]);
        
                $mailNote = ' O pedido foi registrado, mas o email não foi enviado: ' . $mailError->getMessage();
            }
        
            flash('Pedido de IF Coin enviado.' . $mailNote);
            redirect_to('painel.php?page=conta&request_sent=1');
        }        
        ...
        if ($action === 'delete_user') {
            ...
            $pdo->prepare('DELETE FROM coin_requests WHERE requester_id = ? OR target_id = ?')
                ->execute([$id, $id]);
        ...
        if ($action === 'daily_goal_reward') {
            ...
            $amount = max(0, (int) ($_POST['amount'] ?? 0));
            $reason = trim((string) ($_POST['reason'] ?? 'META DIÁRIA BATIDA - RECARGA AUTOMÁTICA'));
            ...
            if (!is_daily_sales_goal_vendor_user($recipient)) {
                throw new RuntimeException('Meta diária batida só pode premiar vendedores cadastrados na meta automática.');
            }

            db()->prepare('UPDATE users SET balance = balance + ?, updated_at = ? WHERE id = ?')
                ->execute([$amount, now(), $id]);
            db()->prepare('INSERT INTO credit_logs (user_id, admin_id, amount, reason, created_at) VALUES (?, ?, ?, ?, ?)')
                ->execute([$id, (int) $admin['id'], $amount, $reason === 'META DIÁRIA BATIDA - RECARGA AUTOMÁTICA' ? $reason : 'Meta diária: ' . $reason, now()]);
...
{
    $emails = daily_sales_goal_vendor_emails();
    if (!$emails) {
        return [];
    }

    $placeholders = implode(',', array_fill(0, count($emails), '?'));
    $stmt = db()->prepare("SELECT * FROM users WHERE active = 1 AND email IN ($placeholders) ORDER BY name");
    $stmt->execute($emails);
...
run_daily_sales_goal_rewards_if_needed();
...
<head>
    ...
    <link rel="stylesheet" href="assets/styles.css?v=3434-00-users-madasdobile-card-final">
...
<div class="shell">
    ...
    <?php elseif ($page === 'conta' && $user): ?>
        ...
        $prefillDonateTo = strtoupper(trim((string) ($_GET['donate_to'] ?? '')));
        $prefillAmount = max(1, (int) ($_GET['amount'] ?? 1));
        $prefillMessage = trim((string) ($_GET['message'] ?? 'Pedido atendido!'));
        $prefillRequestId = max(0, (int) ($_GET['request_id'] ?? 0));
        ?>        
        ...
        <main class="account-layout account-layout-wide">
            ...
            <section class="panel donate-panel">
                ...
                <form method="post" class="form-grid donation-form" data-confirm="Confirmar doação de IF Coins?">
                    ...
                    <label>Código da pessoa
                        <input name="recipient_code" maxlength="5" required placeholder="A1B2C" autocomplete="off" value="<?= e($prefillDonateTo) ?>">
                    </label>
                    
                    <label>Quantidade
                        <input type="number" name="amount" min="1" max="<?= (int) $freshUser['balance'] ?>" required placeholder="Ex: 1" value="<?= $prefillDonateTo !== '' ? (int) $prefillAmount : '' ?>">
                    </label>
                    
                    <label class="full-span">Mensagem opcional
                        <input name="message" maxlength="180" value="<?= e($prefillDonateTo !== '' ? $prefillMessage : 'Mandou bem!') ?>" placeholder="Ex: Mandou bem na campanha!">
                    </label>
                    
                    <?php if ($prefillRequestId > 0): ?>
                        <input type="hidden" name="request_id" value="<?= (int) $prefillRequestId ?>">
                    <?php endif; ?>
                    <button type="submit" <?= (int) $freshUser['balance'] <= 0 ? 'disabled' : '' ?>>Doar IF Coins</button>
                </form>
            </section>
            
            <section class="panel request-coin-panel">
                <div class="panel-heading">
                    <div>
                        <div class="eyebrow">Pedir moeda</div>
                        <h2>Pedir 1 IF Coin para alguém</h2>
                    </div>
                    <span class="badge-soft">kkkk</span>
                </div>
            
                <p>Escolha uma pessoa pelo código, mande uma mensagem e uma figurinha. Ela recebe um email com botão para doar 1 IF Coin, mas precisa confirmar antes.</p>
            
                <form method="post" class="form-grid request-coin-form" data-confirm="Enviar pedido de 1 IF Coin?">
                    <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
                    <input type="hidden" name="action" value="request_coins">
            
                    <label>Código de quem vai receber o pedido
                        <input name="target_code" maxlength="5" required placeholder="A1B2C" autocomplete="off">
                    </label>
            
                    <label>Figurinha
                        <select name="sticker_key">
                            <?php foreach (coin_request_stickers() as $stickerKey => $sticker): ?>
                                <option value="<?= e($stickerKey) ?>"><?= e($sticker['label']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </label>
            
                    <label class="full-span">Mensagem
                        <input name="message" maxlength="180" value="Me doa 1 IF Coin? Nunca te pedi nada... hoje pedi kkkkk" placeholder="Escreva seu pedido">
                    </label>
            
                    <button type="submit">Pedir 1 IF Coin</button>
                </form>
            </section>            

            <?php
            $adjustableTransactions = adjustable_transactions_for_user((int) $freshUser['id'], 12);
            $replacementProducts = all_products(true);
            ?>
            <section class="panel adjust-redemption-panel">
                <div class="panel-heading">
                    <div>
                        <div class="eyebrow">Ajustar resgate</div>
                        <h2>Troca por item disponível</h2>
                    </div>
                    <span class="badge-soft">Ops</span>
                </div>
                <p>Resgatou Monster, chegou lá e só tinha Bally? Ajuste aqui. O item que não tinha continua baixado e o sistema desconta o produto que você realmente pegou.</p>

                <form method="post" class="form-grid adjust-redemption-form" data-confirm="Confirmar ajuste do resgate?">
                    <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
                    <input type="hidden" name="action" value="adjust_redemption">

                    <label>O que você tinha resgatado
                        <select name="transaction_id" required <?= !$adjustableTransactions ? 'disabled' : '' ?>>
                            <option value="">Selecionar resgate</option>
                            <?php foreach ($adjustableTransactions as $tx): ?>
                                <option value="<?= (int) $tx['id'] ?>">
                                    <?= e($tx['product_name']) ?> — <?= e(date('d/m H:i', strtotime($tx['created_at']))) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </label>

                    <label>O que você pegou no lugar
                        <select name="new_product_id" required <?= !$replacementProducts ? 'disabled' : '' ?>>
                            <option value="">Selecionar produto</option>
                            <?php foreach ($replacementProducts as $productOption): ?>
                                <option value="<?= (int) $productOption['id'] ?>">
                                    <?= e($productOption['name']) ?> — <?= (int) $productOption['stock'] ?> em estoque — <?= (int) $productOption['cost'] ?> IF Coin<?= (int) $productOption['cost'] > 1 ? 's' : '' ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </label>

                    <button type="submit" <?= (!$adjustableTransactions || !$replacementProducts) ? 'disabled' : '' ?>>Ajustar resgate</button>
                </form>
            </section>
            ...
            <section class="panel">
                ...
                <div class="table-wrap">
                    <table>
                        <thead><tr><th>Data</th><th>Produto</th><th>Custo</th><th>Status</th></tr></thead>
                        <tbody>
                        <?php
                        ...
                        <?php foreach ($myTransactions as $row): ?>
                            <tr>
                                ...
                                <td data-label="Produto">
                                    <?= e($row['product_name']) ?>
                                    <?php if (!empty($row['original_product_name'])): ?>
                                        <br><small>Era: <?= e((string) $row['original_product_name']) ?></small>
                                    <?php endif; ?>
                                </td>
                                <td data-label="Custo"><?= (int) $row['cost'] ?> IF Coin<?= (int) $row['cost'] > 1 ? 's' : '' ?></td>
                                <td data-label="Status"><?= !empty($row['adjusted_at']) ? 'Ajustado' : 'Normal' ?></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                    </table>
            ...
            <section class="panel">
                <h2>Pedidos de IF Coins</h2>
                <?php render_coin_requests_table(coin_requests_for_user((int) $freshUser['id'], 20), (int) $freshUser['id']); ?>
            </section>            
            ...
            <section class="admin-content">
                ...
                <?php elseif ($tab === 'users'): ?>
                    ...
                    <section class="panel daily-goal-panel">
                        ...
                        <p>Escolha um vendedor da lista, defina a quantidade de IF Coins e escreva o motivo. Essa premiação é exclusiva para vendedores que batem meta.</p>
                        <form method="post" class="form-grid daily-goal-form" data-confirm="Adicionar IF Coins por meta diária?">
                            ...
                            <label class="full-span">Motivo
                                <input name="reason" maxlength="160" required value="META DIÁRIA BATIDA - RECARGA AUTOMÁTICA" placeholder="Ex: META DIÁRIA BATIDA - RECARGA AUTOMÁTICA">
                            </label>
...
function coin_requests_for_user(int $userId, int $limit = 20): array
{
    $stmt = db()->prepare('
        SELECT *
        FROM coin_requests
...
function render_coin_requests_table(array $rows, int $viewerId): void
{
    ?>
    <div class="table-wrap">
        <table>
            ...
            <tbody>
            <?php if (!$rows): ?>
                <tr><td colspan="5">Nenhum pedido de IF Coin registrado ainda.</td></tr>
            <?php endif; ?>
            ...
            <?php foreach ($rows as $row): ?>
                <?php
                    ...
                    $direction = $isRequester
                        ? 'Você pediu para ' . $row['target_name']
                        : $row['requester_name'] . ' pediu para você';
                    ...
                    $emailLabel = strpos((string) $row['email_status'], 'sent') === 0
                        ? 'Enviado'
                        : e((string) $row['email_status']);
                ?>
                <tr>
                    <td data-label="Data"><?= e(date('d/m/Y H:i', strtotime($row['created_at']))) ?></td>
                    <td data-label="Pedido">
                        <strong><?= e($direction) ?></strong><br>
                        <small><?= e((string) $row['message']) ?></small>
                    </td>
                    <td data-label="Coins"><?= (int) $row['amount'] ?></td>
                    <td data-label="Status"><?= e($statusLabel) ?></td>
                    <td data-label="Email"><?= $emailLabel ?></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
...
function adjustable_transactions_for_user(int $userId, int $limit = 12): array
{
    $stmt = db()->prepare('SELECT * FROM transactions WHERE user_id = ? AND adjusted_at IS NULL ORDER BY id DESC LIMIT ?');
    $stmt->bindValue(1, $userId, PDO::PARAM_INT);
            ...
            <?php foreach ($rows as $row): ?>
                <tr>
                    ...
                    <td data-label="Produto">
                        <?= e($row['product_name']) ?>
                        <?php if (!empty($row['original_product_name'])): ?>
                            <br><small>Ajustado — era: <?= e((string) $row['original_product_name']) ?></small>
                        <?php endif; ?>
                    </td>
                    <td data-label="Custo"><?= (int) $row['cost'] ?> IF Coin<?= (int) $row['cost'] > 1 ? 's' : '' ?></td>
                </tr>
